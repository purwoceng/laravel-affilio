@extends('core.app')
@section('title', __('Edit Kategori Utama'))
            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap py-5">
                    <div class="card-title">
                        <h3 class="card-label">Edit Kategori {{ $category->name }}</h3>
                    </div>
                </div>
                            ...
                            <select name="origin_category_id"
                                ...
                                @if (!empty($categories))
                                    @foreach ($categories as $key => $category_data)
                                        <option
                                            value="{{ $category_data['no'] }}"
                                            {{ $category->origin_category_id == $category_data['no'] ? 'selected' : '' }}>
                                            {{ $category_data['name'] }}
                                        </option>
                                    @endforeach
                                @else
