@extends('core.app')
@section('title', __('Header Funnel Home'))
            ...
            <div class="card card-custom">
                ...
                <div class="card-header flex-wrap py-5">
                    <div class="card-title">
                        <h3 class="card-label">Header Funnel Home</h3>
                    </div>
                    <a class="btn btn-success float-right" href="{{ route('headerfunnel.create') }}" title="Tambah Header">
                ...
                <div class="card-body">
                    <table id="js-headers-table" class="table table-striped table-bordered table-sm" cellspacing="0"
                        width="100%">
                        <thead>
                            <tr class="small">
                                <th>#</th>
...
@push('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            const ajaxUrl = "{{ route('headerfunnel.index') }}";

            $('#js-headers-table').DataTable({
                destroy: true,
                processing: true,
                serverSide: true,
                responsive: true,
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            let showUrl =
                                `{{ url('headerfunnel/show/${row.id}') }}`;
                            let editUrl =
                                `{{ url('headerfunnel/edit/${row.id}') }}`;
                            let deleteUrl =
                                `{{ url('headerfunnel/delete/${row.id}') }}`;
                            let elements = '';
